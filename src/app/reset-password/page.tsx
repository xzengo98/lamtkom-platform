...
export default function ResetPasswordPage() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (error) {
      setErrorMessage("تعذر تحديث كلمة المرور. حاول فتح الرابط مرة أخرى.");
      return;
    }
            ...
            <div className="order-2 p-6 md:p-8 xl:order-1 xl:p-10">
              <div className="mx-auto max-w-xl">
                ...
                <p className="mt-3 text-sm leading-8 text-white/58 md:text-base">
                  اختر كلمة مرور جديدة لتأمين حسابك والعودة إلى استخدام المنصة.
                </p>

                {!ready && !successMessage ? (
                  <div className="mt-5 rounded-2xl border border-orange-400/20 bg-orange-500/10 px-4 py-3 text-sm font-bold text-orange-200">
                    إذا كانت الصفحة فارغة من الجلسة، افتح رابط الاستعادة من البريد
                    مرة أخرى مباشرة.
                  </div>
                ) : null}
            ...
            <div className="order-1 border-b border-white/8 bg-[linear-gradient(160deg,rgba(8,16,40,1)_0%,rgba(4,8,22,1)_50%,rgba(6,12,30,1)_100%)] xl:order-2 xl:border-b-0 xl:border-r xl:border-white/8">
              <div className="flex h-full flex-col items-center justify-center px-6 py-12 text-center xl:px-10">
                ...
                <p className="mt-4 max-w-md text-sm leading-8 text-white/58 md:text-base">
                  بعد حفظ كلمة المرور الجديدة ستتمكن من العودة مباشرة إلى تسجيل
                  الدخول ومتابعة حسابك وجولاتك من جديد.
                </p>

                <div className="mt-7 w-full max-w-md space-y-3">
                  {[
                    "تحديث آمن لكلمة المرور.",
                    "استعادة الوصول إلى الحساب بسهولة.",
                    "نفس الهوية البصرية لباقي صفحات الدخول.",
                  ].map((item) => (
                    <div
                      key={item}

...
export default async function EditBaraSectionPage({
  params,
  searchParams,
}: {
  ...
  async function updateSection(formData: FormData) {
    ...
    if (!name) {
      redirect(
        `/admin/bara-alsalfah/categories/edit-section/${resolvedParams.id}?error=${encodeURIComponent(
          "اسم القسم مطلوب",
        )}`,
      );
    }
    ...
    <div className="space-y-6">
      <div className="rounded-[2rem] border border-white/10 bg-[#071126] p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black text-white">تعديل القسم</h1>
            <p className="mt-3 text-white/70">
              عدل بيانات القسم الرئيسية.
            </p>
          </div>

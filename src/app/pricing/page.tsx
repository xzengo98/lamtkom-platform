...
function PlanCard({ plan, index }: { plan: Plan; index: number }) {
  ...
  return (
    <div
      className={[
        "group relative flex flex-col overflow-hidden rounded-[24px] border p-5 transition duration-300",
        "hover:-translate-y-1",
        c.card,
        c.glow,
    ...
    >
      ...
      {isFeatured ? (
        <div className="absolute left-1/2 top-0 z-20 -translate-x-1/2 -translate-y-1/2">
          <div className="rounded-full bg-[linear-gradient(90deg,#a78bfa_0%,#22d3ee_100%)] px-4 py-1.5 text-xs font-extrabold text-white shadow-[0_8px_24px_rgba(34,211,238,0.22)]">
            {plan.badge}
          </div>
        </div>
      ) : null}
      ...
      <div className="relative z-10 flex flex-col">
        <div className="text-center">
          {!isFeatured ? (
            <div
              className={[
                "mx-auto inline-flex items-center rounded-full border px-3 py-1 text-xs font-bold",
                c.badge,
              ].join(" ")}
            >
              {plan.badge}
          ...
          <div className="mt-4">
            <div className={`text-4xl font-black leading-none ${c.price}`}>
              {plan.price}
            </div>
            <div className="mt-1.5 text-xs font-semibold text-white/40">
              السعر
            </div>
          </div>

          {plan.highlight ? (
            <div className="mt-3 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-xs leading-6 text-white/65">
              {plan.highlight}
            </div>
          ) : null}
        </div>
        ...
        <div className="mt-4 space-y-2">
          {plan.features.map((f) => (
            ...
            >
              <span className="text-right text-xs leading-5 text-white/80">
                {f.text}
              </span>

              <span className={`shrink-0 ${c.check}`}>
        ...
        <div className="mt-4">
          <Link
            href={plan.href}
            className={[
              "inline-flex w-full items-center justify-center rounded-xl px-4 py-2.5 text-sm font-black transition",
              c.btn,
            ].join(" ")}
          >
            {plan.cta}

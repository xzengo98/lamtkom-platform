...
const gameCards: GameCardItem[] = [
  {
    ...
    description: "لعبة الفئات والأسئلة الكلاسيكية بين فريقين. اختر فئاتك، راكم النقاط، وتوّج الأذكياء.",
    href: "/game/start",
  ...
  },
  {
    ...
    description: "شخص واحد فقط لا يعرف الكلمة — هل تستطيع كشفه قبل أن يُضلّل الجميع؟",
  ...
  },
  {
    ...
    description: "تلميح واحد يوجّه فريقك لاكتشاف كلماتك — لكن احذر من الكلمة القاتلة.",
...
function GameCard({ card }: { card: GameCardItem }) {
  ...
  return (
    <div className={`group relative flex flex-col overflow-hidden rounded-[2rem] border border-white/8 bg-[linear-gradient(160deg,rgba(12,20,44,0.97)_0%,rgba(5,10,24,0.99)_100%)] transition duration-300 hover:-translate-y-1 ${a.glow}`}>
      {/* Top accent */}
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="absolute bottom-3 right-3">
          <span className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[11px] font-black backdrop-blur-sm ${a.badge}`}>
      ...
      <div className="flex flex-1 flex-col p-5">
        <div className={`mb-1 text-[11px] font-bold uppercase tracking-widest ${a.tag}`}>{card.subtitle}</div>
        <h3 className="text-2xl font-black text-white">{card.title}</h3>
        <p className="mt-2.5 flex-1 text-sm leading-7 text-white/50">{card.description}</p>

        {/* Meta tags */}
        <div className="mt-4 flex flex-wrap gap-2">
          <span className={`inline-flex items-center gap-1 rounded-full border px-3 py-1 text-[11px] font-bold ${a.tag}`}>
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    async function loadState() {
      ...
      if (!mounted) return;
      if (!user) { setIsLoggedIn(false); setUsername(null); setLoading(false); return; }
      const { data: profile } = await supabase.from("profiles").select("username").eq("id", user.id).single();
      if (!mounted) return;
      setIsLoggedIn(true);
      setUsername((profile as Profile | null)?.username ?? null);
      setLoading(false);
    }

    loadState();
    const { data: { subscription } } = supabase.auth.onAuthStateChange(() => loadState());
    ...
    <main className="min-h-screen bg-[linear-gradient(180deg,#020a1a_0%,#030d22_55%,#020814_100%)] text-white">
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-8 md:px-6">
            ...
            <div className="flex flex-col items-center px-6 py-14 text-center xl:items-start xl:px-14 xl:py-18 xl:text-right">
              ...
              <div className="mb-7 xl:hidden">
                <div className="relative mx-auto flex h-28 w-28 items-center justify-center rounded-[1.8rem] border border-cyan-400/18 bg-[radial-gradient(circle_at_center,rgba(34,211,238,0.10),transparent_70%)]">
                  <img src={heroLogo} alt="لمتكم" className="h-20 w-20 object-contain drop-shadow-[0_0_24px_rgba(34,211,238,0.20)]" />
                </div>
              </div>
              ...
              <div className="mt-8 flex flex-wrap justify-center gap-3 xl:justify-start">
                ...
                >
                  <svg viewBox="0 0 24 24" fill="none" className="h-4 w-4" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><rect x="4" y="8" width="16" height="8" rx="3"/><path d="M8 12h2M9 11v2M16.5 12h.01M18.5 12h.01"/></svg>
                  استكشف الألعاب
                </Link>

                {loading ? (
                  <span className="inline-flex items-center gap-2 rounded-xl border border-white/8 bg-white/4 px-6 py-3.5 text-sm font-bold text-white/30">
                    <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-white/30" />
                    جارٍ التحميل...
                  </span>
                ) : isLoggedIn ? (
                  <Link href="/account" className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/6 px-6 py-3.5 text-sm font-black text-white/80 transition hover:bg-white/10">
                  ...
                  <>
                    <Link href="/register" className="inline-flex items-center rounded-xl border border-white/10 bg-white/6 px-6 py-3.5 text-sm font-black text-white transition hover:bg-white/10">
                      إنشاء حساب
                    </Link>
                    <Link href="/login" className="inline-flex items-center rounded-xl border border-white/6 bg-white/3 px-6 py-3.5 text-sm font-bold text-white/55 transition hover:bg-white/7 hover:text-white">
                      تسجيل الدخول
                    </Link>
                  </>
                )}
              ...
              <div className="mt-6 flex flex-wrap justify-center gap-2 xl:justify-start">
                {[
                  ...
                  <span key={pill.label} className="inline-flex items-center gap-1.5 rounded-full border border-white/8 bg-white/[0.035] px-3.5 py-1.5 text-[11px] font-bold text-white/40">
                    {pill.icon}
                    {pill.label}
                  </span>
                ))}
            ...
            <div className="hidden items-center justify-center px-10 py-16 xl:flex">
              <div className="relative">
                ...
                <div className="relative flex h-[310px] w-[310px] items-center justify-center overflow-hidden rounded-[2.4rem] border border-cyan-400/16 bg-[linear-gradient(160deg,rgba(10,20,48,0.98)_0%,rgba(4,8,22,0.99)_100%)] shadow-[0_30px_80px_rgba(0,0,0,0.60)]">
                  ...
                  <img src={heroLogo} alt="شعار لمتكم" className="relative h-[220px] w-[220px] object-contain drop-shadow-[0_0_40px_rgba(34,211,238,0.22)]" />
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="mb-16">
          <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
            {[
              ...
              <div key={stat.label} className={`overflow-hidden rounded-2xl border ${stat.color.pill.split(" ").find(c => c.startsWith("border-")) || "border-white/8"} bg-gradient-to-br ${
                stat.color.bar === "bg-cyan-400"    ? "from-cyan-400/6 to-transparent"    :
                stat.color.bar === "bg-orange-400"  ? "from-orange-400/6 to-transparent"  :
                stat.color.bar === "bg-violet-400"  ? "from-violet-400/6 to-transparent"  :
                ...
                <div className="px-4 py-4">
                  ...
                  <div className={`mt-2 text-2xl font-black ${
                    stat.color.bar === "bg-cyan-400"    ? "text-cyan-300"    :
                    stat.color.bar === "bg-orange-400"  ? "text-orange-300"  :
                    stat.color.bar === "bg-violet-400"  ? "text-violet-300"  :
                                                           "text-emerald-300"
                  }`}>{stat.value}</div>
                </div>
              </div>
        ...
        <section className="mb-16">
          <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className="mt-3 text-2xl font-black text-white md:text-3xl">اختر اللعبة التي تناسب جلستكم</h2>
            ...
            <Link href="/games" className="inline-flex shrink-0 items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-5 py-2.5 text-sm font-black text-white/60 transition hover:bg-white/8 hover:text-white">
              جميع الألعاب
              <ArrowLeft className="h-4 w-4" />
            </Link>
          </div>
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {gameCards.map((card) => <GameCard key={card.title} card={card} />)}
          </div>
        </section>
        ...
        <section className="mb-16">
          ...
          <h2 className="mt-3 text-2xl font-black text-white md:text-3xl">ثلاث خطوات للبدء</h2>
          <p className="mt-2 max-w-xl text-sm leading-7 text-white/45">
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            {howItWorksSteps.map((step, i) => {
              ...
              return (
                <div key={i} className="group relative overflow-hidden rounded-2xl border border-white/8 bg-[linear-gradient(160deg,rgba(10,18,42,0.94)_0%,rgba(4,8,22,0.98)_100%)] p-6 transition duration-200 hover:-translate-y-0.5">
                  <div className={`absolute inset-x-0 top-0 h-[2px] ${a.bar} opacity-60`} />
                  <div className="mb-5 flex items-center gap-3">
                    <div className={`flex h-12 w-12 items-center justify-center rounded-2xl border text-xl font-black ${a.num}`}>
                      {step.num}
                    </div>
                  </div>
        ...
        <section className="mb-16">
          ...
          <h2 className="mt-3 text-2xl font-black text-white md:text-3xl">مصممة لجلستكم</h2>
          <p className="mt-2 max-w-xl text-sm leading-7 text-white/45">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {features.map((f, i) => {
              ...
              const borderHover = f.color === "cyan" ? "hover:border-cyan-400/20" : f.color === "orange" ? "hover:border-orange-400/20" : f.color === "violet" ? "hover:border-violet-400/20" : "hover:border-emerald-400/20";
              return (
                <div key={i} className={`group rounded-2xl border border-white/8 bg-white/[0.025] p-5 transition duration-200 hover:bg-white/[0.04] ${borderHover}`}>
                  <div className={`mb-4 inline-flex h-12 w-12 items-center justify-center rounded-xl border text-2xl ${a.feat}`}>
                    {f.icon}
                  </div>
            ...
            <div className="relative grid gap-8 px-8 py-12 md:grid-cols-[1fr_auto] md:items-center md:gap-12 md:px-12">
              <div>
                ...
                <div className="mb-6 flex items-center gap-3">
                  {gameCards.map((g) => (
                    <div key={g.title} className={`overflow-hidden rounded-xl border ${accentConfig[g.accent].badge.split(" ").find(c => c.startsWith("border-")) || "border-white/10"}`}>
          ...
          <div className="px-7 py-9 md:px-10">
            ...
            <div className="mt-5 flex flex-col items-center gap-3 text-xs text-white/18 md:flex-row md:justify-between">
              ...
              <div className="flex gap-6">
                <Link href="/terms"   className="transition hover:text-white/40">الشروط والأحكام</Link>
                <Link href="/privacy" className="transition hover:text-white/40">الخصوصية</Link>
              </div>
            </div>
          </div>
        </footer>

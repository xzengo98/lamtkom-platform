...
function GamesIcon({ className = "h-4 w-4" }: { className?: string }) {
  return <svg viewBox="0 0 24 24" fill="none" className={className} stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="4" y="8" width="16" height="8" rx="3"/><path d="M8 12h2M9 11v2M16.5 12h.01M18.5 12h.01"/></svg>;
}
function UserIcon({ className = "h-4 w-4" }: { className?: string }) {
  return <svg viewBox="0 0 24 24" fill="none" className={className} stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="3.5"/><path d="M5 19a7 7 0 0 1 14 0"/></svg>;
}
function LogoutIcon({ className = "h-4 w-4" }: { className?: string }) {
  return <svg viewBox="0 0 24 24" fill="none" className={className} stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M15 3h3a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-3"/><path d="M10 17 15 12 10 7"/><path d="M15 12H4"/></svg>;
}
...
function PricingIcon({ className = "h-4 w-4" }: { className?: string }) {
  return <svg viewBox="0 0 24 24" fill="none" className={className} stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 1v22M17 5.5c0-1.9-2.2-3.5-5-3.5S7 3.6 7 5.5 9.2 9 12 9s5 1.6 5 3.5S14.8 16 12 16s-5 1.6-5 3.5"/></svg>;
}
...
export default function Navbar() {
  ...
  const [authState,  setAuthState]  = useState<AuthState>({
    loading: true, isLoggedIn: false, isAdmin: false, username: null,
  });

  // ── Auth loading (logic unchanged) ──
  async function loadUser() {
    const { data: { user } } = await supabase.auth.getUser();

    if (!user) {
      setAuthState({ loading: false, isLoggedIn: false, isAdmin: false, username: null });
      return;
    }
  ...
  const navLinks = [
    ...
    { label: "الألعاب",  href: "/games",   icon: <GamesIcon className="h-3.5 w-3.5" /> },
    { label: "الباقات",  href: "/pricing", icon: <PricingIcon className="h-3.5 w-3.5" /> },
  ];
    ...
    <header className="relative z-40 border-b border-white/8 bg-[#040c1e]/96 backdrop-blur-xl">
      ...
      <div className="mx-auto max-w-7xl px-4 md:px-6">
        <div className="flex min-h-[72px] items-center justify-between gap-4 sm:min-h-[80px]">
          ...
          {/* Logo */}
          <Link href="/" className="flex shrink-0 items-center gap-3 transition-opacity hover:opacity-85">
            <img
          ...
          <div className="border-t border-white/8 pb-4 pt-3 lg:hidden">
            ...
            <div className="mt-3 flex flex-col gap-2 border-t border-white/6 pt-3">
              {authState.loading ? (
                ...
                <>
                  <Link href="/register" className="inline-flex items-center justify-center rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-bold text-white/70 transition hover:bg-white/8">
                    إنشاء حساب
                  </Link>
                  <Link href="/login" className="inline-flex items-center justify-center rounded-xl bg-cyan-500 px-4 py-3 text-sm font-black text-slate-950 shadow-[0_2px_12px_rgba(34,211,238,0.22)] transition hover:bg-cyan-400 active:scale-[0.98]">
                    تسجيل الدخول
                  </Link>
                </>
              )}
